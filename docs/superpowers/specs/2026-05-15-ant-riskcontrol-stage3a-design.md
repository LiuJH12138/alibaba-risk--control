...
Stage 2 完成模型基础升级后,deep best PR-AUC = 0.864,LightGBM = 0.902,差 ~4 个百分点。Stage 2 给出诚实结论:**仅特征工程升级不足以反超 LGB**。Stage 3a 假设差距来自 Stage 2 同构图的两个结构性缺陷:
...
- hetero best > LGBM 0.902 → 反超达成 ✅
- hetero best ∈ (Stage 2 deep 0.864, 0.902) → 异质图有效但未超 LGB
- hetero best ≈ Stage 2 deep 0.864 → 异质图在本数据集帮助有限
- hetero best < Stage 2 deep 0.864 → 实现需排查或同构已够
...
**对照基准**(直接复用 Stage 2 数字):

- Stage 2 `deep_pruned` (homo + HybridFocal,gated): PR-AUC = 0.864 ← homo 基线
- Stage 2 `lgbm_pruned`: PR-AUC = 0.902 ← 传统模型上限
...
|------|-----------|-------------|--------|-----------|------------------|
| Stage 2 deep_pruned (对照) | 0.864 | 0.948 | 0.781 | ✅ | (Stage 2 已有) |
...
| Stage 2 lgbm_pruned (对照) | 0.902 | TBD | TBD | ✅ | (Stage 2 已有) |

## 诚实四情景结论(实测后选其一)
- [ ] hetero best > LGBM 0.902:深度反超 ✅
- [ ] hetero best ∈ (0.864, 0.902):异质图有效但未超 LGB
- [ ] hetero best ≈ 0.864:异质图帮助有限
- [ ] hetero best < 0.864:实现需排查
...
- [ ] hetero best PR-AUC ≥ Stage 2 deep_pruned 0.864
- [ ] 至少 1 个配置 converged=True 且 PR-AUC ≥ 0.85
- [ ] core_entities top-10 中 train_fraud_rate ≥ 0.5 的 entity 占比 ≥ 50%

**绝不放进 DoD 的项**(诚实原则):

- ❌ "PR-AUC 必须 > 0.902"(不能为赢 LGBM 而调超参)
